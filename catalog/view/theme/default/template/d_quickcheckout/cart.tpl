	...
	<div class="panel panel-default <%= parseInt(model.config.display) ? '' : 'hidden' %>">
		...
		<div class="qc-checkout-product panel-body" >
			...
			<table class="table table-bordered qc-cart">
				<thead>
					<tr>
						<td class="qc-image hidden-xs <%= parseInt(model.config.columns.image) ? '' : 'hidden' %>"><?php echo $column_image; ?>:</td>
				...
				<tbody>
					...
					<tr <%= product.stock ? '' : 'class="stock"' %>>
						<td class="qc-image hidden-xs <%= parseInt(model.config.columns.image) ? '' : 'hidden' %>">
