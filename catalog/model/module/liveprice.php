...
class ModelModuleLivePrice extends Model {
  ...
  private $io_installed_status = null;
	...
	private $customer_group_price_status = null;
  ...
  public function installedRelatedOptions() {
	...
	// Customer Group Priceby Jade Agile
	public function installedCustomerGroupPrice() {
    if ( is_null($this->customer_group_price_status) ) {
			$query_pc = $this->db->query("SHOW TABLES LIKE '".DB_PREFIX."product_price' ");
      $query_pco = $this->db->query("SHOW TABLES LIKE '".DB_PREFIX."product_priceoption' ");
      $this->customer_group_price_status = $query_pc->num_rows && $query_pco->num_rows;
    }
    return $this->customer_group_price_status;
  }
  ...
  public function getThemeName() {
	...
	protected function getCustomPHPCodeFile() {
		$file_name = DIR_TEMPLATE.'../extension/live_price/themes/'.$this->getThemeName().'/custom_code.php';
	...
	protected function loadTemplate($route, $data) {
		...
		if ( class_exists('Template') ) {
			if ( $this->registry->get('config')->get('template_type') && $this->registry->get('config')->get('template_type') ) {
				$template_type = $this->registry->get('config')->get('template_type');
			} else {
				$template_type = 'basic';
			}
		}
		
		if ( class_exists('Template\\'.$template_type) ) {
			
			$template = new Template($template_type);
    ...
    if ( $this->installed() && $this->versionPRO() ) {
      ...
      if ( !empty($lp_settings['default_price']) ) { // defaults from IO and RO has higher priority than lowest price of option
        ...
        if ( $this->installedImprovedOptions() ) {
          ...
          if (!$this->model_module_improvedoptions) {
  ...
  private function ProductHasSizeOption($options, $options_types) {
	...
	private function getCustomerGroupProductOptionPrice($pov_id, $price) {
		if ( $this->installedCustomerGroupPrice() ) {
			$product_priceoption_query = $this->db->query("
				SELECT *
				FROM " . DB_PREFIX . "product_priceoption
				WHERE product_option_value_id='". (int)$pov_id ."'
					AND customer_group_id='". (int)$this->config->get('config_customer_group_id') ."'
			");
			if($product_priceoption_query->row) {
				$price = $product_priceoption_query->row['price'];
			} 
		}
		return $price;
	}
	
	private function getCustomerGroupProductPrice($product_id, $price) {
		if ( $this->installedCustomerGroupPrice() ) {
			$product_price_query = $this->db->query("
				SELECT price
				FROM " . DB_PREFIX . "product_price
				WHERE product_id = ".(int)$product_id."
					AND customer_group_id = ".(int)$this->config->get('config_customer_group_id')."
			");
			if($product_price_query->row) {
		...
		if ( count($product_option_ids) != 0 ) {
			...
			if ( count($product_option_ids) != 0 && count($product_option_value_ids) != 0 ) { // в $product_option_ids могут быть опции не подходящих типов
				...
				foreach ($option_value_query->rows as $row) {
					...
					$row['price'] = $this->getCustomerGroupProductOptionPrice($row['product_option_value_id'], $row['price']);
  ...
  public function getProductPrice($p_product_id, $current_quantity=1, $options=array(), $recurring_id=0, $prices=array(), $product_data=array(), $option_data=array(), $multiplied_price=false, $use_cart_cache=false, $use_price_cache=false, $without_discounts=false, $ignore_cart=false, $qpo_discount_quantity_addition=0) {
    ...
    if ( $use_price_cache && isset($this->cache_price[$cache_price_key]) ) {
      ...
      if ($product_query->num_rows) {
				...
				$product_query->row['price'] = $this->getCustomerGroupProductPrice($product_id, $product_query->row['price']);
        ...
        $product_data = array(
          ...
          'recurring'       => $recurring
				...
				$prices['weight'] = $product_query->row['weight'] + $option_weight;
  ...
  public function getProductPriceWithHtml($product_id, $current_quantity=0, $options=array(), $prices=array(), $product_data=array(), $option_data=array(), $multiplied_price=false, $non_standard_theme='' ) {
    ...
    $simple_prices = array( 'price'       =>  $prices['f_price_old_opt'],
                            ...
                            'product_id'  =>  $product_id,
														'tax_class_id'=> 	$prices['tax_class_id'],
                            );
	...
	public function getProductTotalPriceForQuantityPerOptionWithHtml($p_product_id, $p_options, $p_quantity_per_options, $p_non_standard_theme) {
		...
		if ( $total_quantity ) {
			...
			$simple_prices = array( 'price'       =>  $this->format( $this->tax->calculate($total_price_old_opt, $current_tax_class_id, $current_config_tax) ), //$prices['f_price_old_opt']
															...
															'tax_class_id'=>	$current_tax_class_id,
														);
  ...
  function getPriceHtmls($prices, $non_standard_theme='') {
    ...
    $custom_templates = $this->getCustomTemplates();
		...
		if ( !empty($custom_templates) ) {
			...
			$custom_php_file = $this->getCustomPHPCodeFile();
			if ( $custom_php_file ) {
				//print $custom_php_file;
				include($custom_php_file);
			}
      ...
      if (!$special) {
        ...
        $html1 .= '<li> <span class="text-price"> '.$special.' </span> <span style="text-decoration: line-through;">'. $price.'</span> </li>';
			...
			if ($tax) {
        $html1 .= '<li class="other-price">'.$text_tax.' '.$tax.'</li>';
			}
    ...
    // OPC080178 constitute theme
      ...
      */  
      ...
      require_once($nico_include_path . 'nico_theme_editor/common.inc');
      //require($nico_include_path . 'nico_theme_editor/nico_config.inc');
			require($nico_include_path . 'nico_config.inc'); // lvpncrt
