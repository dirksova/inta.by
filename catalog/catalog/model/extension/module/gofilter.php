...
public function getGeneraionSql($common_value_id, $stockStock) {
	...
	if(isset($common_value_id['categ_id'])) {$common_value_id['category_id'] = $common_value_id['categ_id'];}
		...
		if (isset($common_value_id['option_filter'])) {
			...
			foreach ($common_value_id['option_filter'] as $product_option_id => $value) {
				...
				if ($kolvo_option == 0) {
					$sql .= " x.option_value_id = '" . (int)$value . "' AND x.quantity <> 0";
				} else {
					$sql .= $sql_compare;
		...
		if (isset($common_value_id['filter_filter'])) {
			...
			$kolvo_option = 0;
			foreach ($common_value_id['filter_filter'] as $product_filter_id => $value) {
				
				if ($kolvo_option == 0) {
					$sql .= " f.product_id IN (SELECT f2.product_id  FROM " . DB_PREFIX . "product_filter f2 WHERE f2.filter_id = '" . (int)$value . "')";
				} else {
					$sql .= $sql_compare;
					$sql .= " f.product_id IN (SELECT f2.product_id  FROM " . DB_PREFIX . "product_filter f2 WHERE f2.filter_id = '" . (int)$value . "')";
				}
...
public function getParameterProducts($common_value_id, $quantity_view, $stockStock, $status_massivs) {

		$sql = $this->getGeneraionSql($common_value_id, $stockStock);
...
public function getSeoElements($filter_array) {
	...
	public function getidProducts($common_value_id, $stockStock) {
		...
		if (isset($common_value_id)) {$temp_common_value_id = $common_value_id; unset($temp_common_value_id['rating_filter']); unset($temp_common_value_id['sort']); unset($temp_common_value_id['order']); unset($temp_common_value_id['limit']); unset($temp_common_value_id['page']);unset($temp_common_value_id['go_mobile']);unset($temp_common_value_id['_route_']);unset($temp_common_value_id['patfilter']);} else {$temp_common_value_id = false;}
...
public function getOptionsProducts($common_value_id) {
	...
	$query_products = $this->getidProducts($common_value_id, $stockStock);
...
public function getCommonProducts($category, $quantity_view, $getCache, $stockStock, $status_massivs) {
		...
		if ($category) {
			$this->db->query("CREATE TEMPORARY TABLE IF NOT EXISTS store_temp_table (INDEX indextemp (product_id)) AS (SELECT p.product_id,p.manufacturer_id,p.price,p.tax_class_id,p.quantity,p.stock_status_id FROM " . DB_PREFIX . "product_to_category ptc LEFT JOIN " . DB_PREFIX . "category_path cp ON (ptc.category_id = cp.category_id) INNER JOIN " . DB_PREFIX . "category_to_store AS c2s ON (ptc.category_id = c2s.category_id) INNER JOIN " . DB_PREFIX . "product_to_store AS p2sq ON (ptc.product_id = p2sq.product_id) INNER JOIN " . DB_PREFIX . "product p ON (p.product_id = p2sq.product_id) WHERE p2sq.store_id = '0' AND c2s.store_id = '0' AND p.status = '1' AND IF(cp.path_id IS NOT NULL, cp.path_id, ptc.category_id) = '" . (int)$category . "'" . $sql_quantity . ")");
			...
			$this->db->query("CREATE TEMPORARY TABLE IF NOT EXISTS store_temp_table (INDEX indextemp (product_id)) AS (SELECT p.product_id,p.manufacturer_id,p.price,p.tax_class_id,p.quantity,p.stock_status_id FROM " . DB_PREFIX . "product p INNER JOIN " . DB_PREFIX . "product_to_store AS p2sq ON (p.product_id = p2sq.product_id) WHERE p2sq.store_id = '0' AND p.status = '1'" . $sql_quantity . ")");
					...
					foreach ($temp_option_id_data as $temp_key => $temp_value) {
						...
						foreach ($temp_value as $key => $value) {
							if ($value['option_value_total']) {
								...
								$sort_name[$key] = $value['option_value_name'];
							}
						}
						array_multisort($sort_name, SORT_ASC, $option_values);
						$common_value_data['options'][] = array(
							'option_id'          	   => $temp_key,
							'name'          	       => $option_name,
							'type'          		   => $option_type,
		...
		if (isset($status_massivs['status_attributes'])) {
			...
			$sql_attributes = "SELECT pa.attribute_id,pa.text as rtext" . ($quantity_view ? ',COUNT(DISTINCT p.product_id) AS total' : ',-1 as total') . " FROM " . $text_sql_category_left_join . " p INNER JOIN " . DB_PREFIX . "product_attribute pa ON (pa.product_id = p.product_id) WHERE pa.text <> '' AND pa.text <> ' ' AND pa.language_id = '" . (int)$this->config->get('config_language_id') . "' GROUP BY pa.attribute_id,rtext";
		...
		if (isset($status_massivs['status_manufacturers'])) {
			$sql_manufacturers = "SELECT p.manufacturer_id,m.name,m.image" . ($quantity_view ? ',COUNT(DISTINCT p.product_id) as total' : ',-1 as total') . " FROM " . DB_PREFIX . "manufacturer m INNER JOIN" . $text_sql_category_left_join . " p ON (p.manufacturer_id = m.manufacturer_id) GROUP BY p.manufacturer_id";
