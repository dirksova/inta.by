...
class ControllerExtensionModuleGofilter extends Controller {
	...
	public function filter_common() {
		...
		if (isset($this->request->cookie['reset' . $this->session->getId()])) {
			$data['set_cookie'] = $_COOKIE['reset' . $this->session->getId()];
		} else {
		...
		$massivs_status = array('status_price','status_category','status_keywords','status_manufacturers','status_stock','status_ratings','status_options','status_attributes','status_filter');
		...
		$massivs_settings = array('popup_view', 'quantity_view', 'stock_quantity', 'id_content', 'id_column_left', 'id_column_right', 'id_button_mobile', 'max_height', 'count_show', 'status_cache', 'status_seo', 'seo_separator', 'filter_scripts');
					...
					if (in_array((int)$this->config->get('config_store_id') . "-" . $layoute_id, $value['layout'])) {
						if ($test_stock_category) {
							...
							if ($data['route'] == "product/category") {
								if (isset($value['categories_layout'])) {
									if (in_array($category_id, $value['categories_layout'])) {
		...
		if ($data['category_id']) {
			$results = $this->model_extension_module_gofilter->getCommonProducts($data['category_id'], $quantity_view, $getCache, $stockStock, $status_massivs);
		} else {
			$results = $this->model_extension_module_gofilter->getCommonProducts(false, $quantity_view, $getCache, $stockStock, $status_massivs);
		}
		...
		if (isset($this->request->get['route_layout']) or isset($this->request->post['route_layout']) or strpos($this->request->server['REQUEST_URI'], "gofilter/") !== false) {
			if ($this->request->get && $this->request->get['route'] != "extension/module/gofilter/live_option_product" && $this->request->get['route'] != "extension/module/gofilter/live_home_filter") {
				$results_product = $this->model_extension_module_gofilter->getParameterProducts($this->request->get, $quantity_view, $stockStock, $status_massivs);
			}
			if ($this->request->post) {
				$results_product = $this->model_extension_module_gofilter->getParameterProducts($this->request->post, $quantity_view, $stockStock, $status_massivs); 
			...
			if (isset($results['categories'])) {
				...
				if (isset($data['categories'])) {
					...
					if (isset($results_product['categories'])) {
						...
						if (isset($sort_type_filter) and isset($data_type_filter)) {array_multisort($data_type_filter, SORT_ASC, $data['categories']);}
					}
				}
				...
				foreach ($results['options'] as $result_option) {
					...
					if ($option_values) {
						...
						if (isset($results_product['options'])) {
							...
							array_multisort($data_type_filter, SORT_NUMERIC, $option_values);
						}
					}
					$options_show_required = true;
					if ($options_empty) {
						foreach ($options_empty as $key_empty_array => $value_empty_array) {
							if ($result_option['option_id'] == $value_empty_array) {
			...
			if (isset($results['attributes'])) {
				...
				foreach ($results['attributes'] as $attribute) {
					...
					foreach ($attribute['attribute'] as $value) {
						...
						$attribute_results_stock_required = true;
						$total_attributes = 0;
						if (isset($results_product)) {
							if (isset($results_product['attributes'])) {
						...
						if ($total_attributes == 0) {$attribute_results_stock_required = false;}
						if ($value['text']) {
							$attribute_values[] = array(
								...
								'attribute_results_stock_required'  => $attribute_results_stock_required
							);
						}
					...
					if ($attribute_values) {
						...
						foreach ($attribute_values as $key => $value) {
							$sort_type_filter[$key] = $value['attribute_results_stock_required'];
							$data_type_filter[$key] = $value['text'];
						}
						if (isset($results_product['attributes'])) {
							array_multisort($sort_type_filter, SORT_DESC, $data_type_filter, SORT_ASC, $attribute_values);
						} else {
					...
					if ($attribute_empty) {
						foreach ($attribute_empty as $key_empty_array => $value_empty_array) {
							if ($value['attribute_id'] == $value_empty_array) {
			...
			if (isset($results['manufacturers'])) {
				...
				if ($data['manufacturers']) {
					...
					if (isset($results_product['manufacturers'])) {
						...
						array_multisort($data_type_filter, SORT_ASC, $data['manufacturers']);
					}
				}
			}
