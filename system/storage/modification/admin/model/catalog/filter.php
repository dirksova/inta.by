...
class ModelCatalogFilter extends Model {
	...
	public function getFilterGroup($filter_group_id) {
		...
		// LMCfilter BEGIN
		$query = $this->db->query("SELECT fg.*, fgd.*, fp.mode as LMCfilter_mode, fp.data as LMCfilter_data FROM `" . DB_PREFIX . "filter_group` fg LEFT JOIN " . DB_PREFIX . "LMCfilter fp ON (fg.filter_group_id = fp.filter_group_id) LEFT JOIN " . DB_PREFIX . "filter_group_description fgd ON (fg.filter_group_id = fgd.filter_group_id) WHERE fg.filter_group_id = '" . (int)$filter_group_id . "' AND fgd.language_id = '" . (int)$this->config->get('config_language_id') . "'");
		// LMCfilter END
	...
	public function getFilterGroups($data = array()) {
		...
		// LMCfilter BEGIN
		$sql = "SELECT fg.*, fgd.*, fp.mode AS LMCfilter_mode FROM `" . DB_PREFIX . "filter_group` fg LEFT JOIN " . DB_PREFIX . "filter_group_description fgd ON (fg.filter_group_id = fgd.filter_group_id) LEFT JOIN oc_LMCfilter fp ON (fg.filter_group_id = fp.filter_group_id) WHERE fgd.language_id = '" . (int)$this->config->get('config_language_id') . "'";
		// LMCfilter END
